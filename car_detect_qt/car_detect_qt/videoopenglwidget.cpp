...
void VideoOpenGLWidget::paintGL()
{
    ...
    // 设置像素存储方式 (非常重要！)
    // 确保 GL 按照紧凑方式读取数据，不进行任何对齐填充
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
    glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0);
    glPixelStorei(GL_UNPACK_SKIP_ROWS, 0);

    // 重新设置 Uniform (防止某些情况下丢失)
    ...
    // 理论上 chunk2_ptr 指向 V 平面 (在 H264Decoder 中强制交换了顺序)
    const uint8_t *chunk2_ptr = y_ptr + m_currentFrame.ySize;
    // 理论上 chunk3_ptr 指向 U 平面
    const uint8_t *chunk3_ptr = chunk2_ptr + m_currentFrame.uSize;

    // Y 分量
    glActiveTexture(GL_TEXTURE0);
    m_textureY->bind();
    m_textureY->setData(QOpenGLTexture::Red, QOpenGLTexture::UInt8, (const void*)y_ptr);

    // U 分量 (纹理单元 1) 接收 U 数据 (即 chunk3)
    // 这是 I420 渲染器的期望顺序 (U在V后面)
    glActiveTexture(GL_TEXTURE1);
    m_textureU->bind();
    m_textureU->setData(QOpenGLTexture::Red, QOpenGLTexture::UInt8, (const void*)chunk3_ptr);

    // V 分量 (纹理单元 2) 接收 V 数据 (即 chunk2)
    // 这是 I420 渲染器的期望顺序 (V在U前面)
    glActiveTexture(GL_TEXTURE2);
    m_textureV->bind();
    m_textureV->setData(QOpenGLTexture::Red, QOpenGLTexture::UInt8, (const void*)chunk2_ptr);
