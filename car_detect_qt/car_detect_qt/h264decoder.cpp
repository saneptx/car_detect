...
//    int ret = av_packet_from_data(_packet, const_cast<uint8_t*>(data), len);
    ...
    while (true) {
        ...
        int w2 = w / 2;
        int h2 = h / 2;
        // 紧凑尺寸：即去除 Padding 后的实际数据大小
        // 紧凑尺寸：即去除 Padding 后的实际数据大小
        int ySize_tight = w * h;
        int uSize_tight = w2 * h2;
        int vSize_tight = w2 * h2;
        int totalSize = ySize_tight + uSize_tight + vSize_tight;
        ...
        // 内存拷贝: V 分量 (放在第二个 chunk 的位置)
        // 使用 _frame->data[2] (V) 填充第二个平面
        uint8_t *destV_chunk = dest + ySize_tight;
        for (int i = 0; i < h2; ++i) {
            memcpy(destV_chunk + i * w2, _frame->data[2] + i * _frame->linesize[2], w2);
        }

        // 内存拷贝: U 分量 (放在第三个 chunk 的位置)
        // 使用 _frame->data[1] (U) 填充第三个平面
        uint8_t *destU_chunk = dest + ySize_tight + vSize_tight;
        for (int i = 0; i < h2; ++i) {
        ...
        // 更新为紧凑大小
        frame.ySize = ySize_tight;
        frame.uSize = vSize_tight; // 对应 V chunk 的大小
        frame.vSize = uSize_tight; // 对应 U chunk 的大小
        //执行回调
        if (onFrame) {
